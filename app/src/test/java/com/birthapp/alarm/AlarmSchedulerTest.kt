...
class AlarmSchedulerTest {
    ...
    private fun solar(
        ...
        calendarType = "solar", advanceDays = listOf(advanceDays),
    ...
    private fun lunar(
        ...
        calendarType = "lunar", isLeapMonth = isLeap, advanceDays = listOf(advanceDays),
        reminderHour = hour, reminderMinute = minute
    )
    ...
    fun `当天提醒_时刻未到_今天触发`() {
        ...
        assertEquals(epoch(2026, 8, 10, 8, 0), calculateNextTriggerTime(b, b.advanceDays.first(), now))
    }
    ...
    fun `当天提醒_时刻已过_顺延到明年同一天`() {
        ...
        assertEquals(epoch(2027, 8, 10, 8, 0), calculateNextTriggerTime(b, b.advanceDays.first(), now))
    }
    ...
    fun `提前一天_在生日前一天触发`() {
        ...
        assertEquals(epoch(2026, 8, 9, 8, 0), calculateNextTriggerTime(b, b.advanceDays.first(), now))
    }
    ...
    fun `提前七天_跨月_三月一日生日在二月二十二日提醒`() {
        ...
        assertEquals(epoch(2026, 2, 22, 8, 0), calculateNextTriggerTime(b, b.advanceDays.first(), now))
    }
    ...
    fun `提前天数大使提醒日已过_顺延到下一年`() {
        ...
        assertEquals(epoch(2027, 8, 5, 8, 0), calculateNextTriggerTime(b, b.advanceDays.first(), now))
    }
    ...
    fun `提前365天_提醒日落在四年的搜索窗口尾部`() {
        ...
        assertEquals(epoch(2027, 1, 1, 8, 0), calculateNextTriggerTime(b, b.advanceDays.first(), now))
    }
    ...
    fun `自定义提醒时刻_23点59分边界`() {
        ...
        assertEquals(epoch(2026, 8, 10, 23, 59), calculateNextTriggerTime(b, b.advanceDays.first(), now))
    }
    ...
    fun `自定义提醒时刻_零点_前一天23点59分后触发`() {
        ...
        assertEquals(epoch(2026, 8, 10, 0, 0), calculateNextTriggerTime(b, b.advanceDays.first(), now))
    }
    ...
    fun `阳历2月29日_平年不存在_按2月28日提醒`() {
        ...
        assertEquals(epoch(2026, 2, 28, 8, 0), calculateNextTriggerTime(b, b.advanceDays.first(), now))
    }
    ...
    fun `阳历2月29日_闰年正常当天提醒`() {
        ...
        assertEquals(epoch(2028, 2, 29, 8, 0), calculateNextTriggerTime(b, b.advanceDays.first(), now))
    }
    ...
    fun `农历腊月生日_跨公历年_正月前命中上一个农历年`() {
        ...
        assertEquals(epoch(2026, 2, 2, 8, 0), calculateNextTriggerTime(b, b.advanceDays.first(), now))
    }
    ...
    fun `农历冬月生日_跨公历年_冬月属于上一个农历年`() {
        ...
        assertEquals(epoch(2026, 1, 3, 8, 0), calculateNextTriggerTime(b, b.advanceDays.first(), now))
    }
    ...
    fun `农历腊月生日_已过_顺延到下一个腊月`() {
        ...
        assertEquals(epoch(2027, 1, 22, 8, 0), calculateNextTriggerTime(b, b.advanceDays.first(), now))
    }
    ...
    fun `闰月生日_当年无闰月_降级为正常月份`() {
        ...
        assertEquals(epoch(2026, 7, 28, 8, 0), calculateNextTriggerTime(b, b.advanceDays.first(), now))
    }
    ...
    fun `闰月生日_当年有闰月_在闰月过`() {
        ...
        assertEquals(epoch(2025, 8, 8, 8, 0), calculateNextTriggerTime(b, b.advanceDays.first(), now))
    }
    ...
    fun `相同输入_多次调用结果一致`() {
        val b = lunar(12, 15)
        val now = LocalDateTime.of(2026, 1, 10, 0, 0)
        val first = calculateNextTriggerTime(b, b.advanceDays.first(), now)
        val second = calculateNextTriggerTime(b, b.advanceDays.first(), now)
    ...
    @Test
    fun `空级别列表_退化为当天`() {
        assertEquals(listOf(0), normalizeAdvanceLevels(emptyList()))
    }

    @Test
    fun `级别列表_去重并升序`() {
        assertEquals(listOf(0, 1, 3, 5), normalizeAdvanceLevels(listOf(5, 1, 5, 0, 3)))
    }

    @Test
    fun `级别列表_越界值被剔除`() {
        assertEquals(listOf(0), normalizeAdvanceLevels(listOf(0, 400, -1)))
    }

    @Test
    fun `级别列表_超过上限只保留前十个`() {
        val levels = (0..15).toList()
        assertEquals(10, normalizeAdvanceLevels(levels).size)
        assertEquals((0..9).toList(), normalizeAdvanceLevels(levels))
    }

    @Test
    fun `单个级别_原样保留`() {
        // v2 老数据迁移来的单值语义：只提前 3 天，不自动展开成 [0,3]
        assertEquals(listOf(3), normalizeAdvanceLevels(listOf(3)))
    }
}
