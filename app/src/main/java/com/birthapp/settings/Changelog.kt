...
object Changelog {
    ...
    val all = listOf(
        ChangelogEntry(
            version = "2.1.19",
            title = "修复核心缺陷：小组件按真实尺寸渲染",
            items = listOf(
                "修复核心缺陷：桌面小组件在 4×2 / 4×4 等任意尺寸下都只渲染 2×2 单焦布局、大组件大面积空白的问题——真实尺寸改由系统读取（Glance 静态兜底值不再参与判型），行数随真实高度自适应：4×2 问候语 + 两行、4×3 三行、4×4 Hero 聚焦头 + 三行，拉得越高显示越多（上限五行），行距均匀铺满整面",
                "4×2 等紧凑高度的行内容自动切小号变体（更小头像/字号/内边距），不会溢出裁切",
                "大尺寸「共 N 个日子」改为真实总记录数，列表只展示最近 N 条时不再误报总数"
            )
        ),
        ChangelogEntry(
            version = "2.1.18",
