...
object Changelog {
    ...
    val all = listOf(
        ChangelogEntry(
            version = "2.1.21",
            title = "修复：英文环境详情页/分享卡崩溃，语言显示不一致",
            items = listOf(
                "修复：英文环境下打开任意详情页必崩溃的问题（日期格式模板参数错位）",
                "修复：英文分享卡片日期行的崩溃风险（模板参数缺失，且两份模板不一致）",
                "修复：Android 13+ 用「应用语言」切换语言后，界面文案与日期格式语言不一致的问题——界面与日期格式现在始终同语言",
                "稳定性：标注小组件尺寸回调的设计意图，消除 lint 误报（无行为变化）"
            )
        ),
        ChangelogEntry(
            version = "2.1.20",
            title = "桌面小组件还原「白底列表」形态 + 应用全面支持中/英文",
            items = listOf(
                "桌面小组件回归经典白底列表：2×2 居中大字（头像 + 名字 + 「N 天后」整句、倒计时按类型配色）、4×2 品牌头 + 右侧「＋」+ 三行弹性列表、大尺寸六行并自动用空白行垫底，空状态两行字提示、点整卡进 App——纸笺式 Hero 渐变头/问候语等旧版式移除",
                "小组件渲染机制保留上一版修复成果：真实尺寸读取、任意桌面尺寸都正常显示，拖动调整后即时重绘",
                "支持中/英文双语：界面文案全部国际化，跟随系统语言自动切换（中文环境与旧版完全一致）；英文环境下日历、通知、分享卡片同步本地化",
                "系统语言非中文/英文时按中文显示，不受影响"
            )
        ),
        ChangelogEntry(
            version = "2.1.19",
