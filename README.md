...
- **首页筛选**：按类型胶囊与关系标签快速过滤，支持置顶固定重要记录
- **月历视图**：列表 / 月历切换，每天农历标注，有记录的日子一目了然
- **提醒通知**：多级提前提醒（如提前 3 天 + 当天），点通知直达记录详情页，重启、改时间后自动重排闹钟
...
- **CI**（`.github/workflows/ci.yml`）：每次 push/PR 自动运行全部单元测试（农历/备份/筛选/事件计算/闹钟调度/迁移 7 个测试类）+ 构建 Debug 包
