...
- **桌面小组件**：经典白底列表形态——2×2 居中大字（头像 + 名字 + 「N 天后」整句、倒计时按类型配色：今天珊瑚/常规青绿/缅怀灰蓝）、4×2「辰记」品牌头 + 右侧「＋」+ 三行弹性列表（行框均分铺满）、大尺寸六行并自动空白行垫底防单条悬空；空态两行字引导、点整卡进 App、按桌面真实尺寸渲染（任意尺寸/拖动调整后即时重绘，不猜格子数）；可配置展示最近记录或指定某人；长按应用图标「添加小组件」或设置页「关于」入口可直接放置（不打开 App，放置成功有提示）
- **中英双语（i18n）**：全部界面文案国际化，跟随系统语言自动切换（系统为中文时与旧版完全一致；英文环境下列表/日历/通知/小组件/分享卡片同步本地化；非中英文系统按中文显示）；农历术语（农历月日名/生肖名）作为文化数据保留中文读法
...
- **CI**（`.github/workflows/ci.yml`）：每次 push/PR 自动运行全部单元测试（农历/备份/筛选/事件计算/闹钟调度/迁移/Compose UI/ViewModel/分层/Hero 候选/小组件布局共 220 用例）+ 构建 Debug 包
