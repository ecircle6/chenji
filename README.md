...
- **首页全新改版**：底部「首页/日历」双页签，日历独立成页展示全部记录；页首问候语 + 最近事件 Hero 聚焦卡（与列表去重、缅怀不放大、≤7 天带进度条）+ 紧凑卡片布局（类型色条 / 类型标签 / 日期·关系）；动效体系——列表错峰入场、倒计时数字滚动、「今天」呼吸光晕
- **多维筛选**：快捷胶囊（全部/关系/类型）+「更多筛选」底部面板，支持关系、类型、生肖三维叠加；支持置顶固定重要记录
- **提醒通知**：多级提前提醒（如提前 3 天 + 当天），点通知直达记录详情页，重启、改时间后自动重排闹钟；通知设置页可调默认提醒时间与总开关
- **桌面小组件**：2×2 / 4×2 / 4×4 三档尺寸，纸笺辰刻排版（左 3.5dp 色条 + 36dp 头像 + 类型标签 + 倒计时，4×4 Hero 与 App 同源渐变头且深色自动切夜间渐变，字重层级数字为焦点，类型色日夜分流对比度达标，缅怀灰蓝、空态「＋添加」按钮，暗色自适配）；可配置展示最近记录或指定某人；长按应用图标「添加小组件」或设置页「关于」入口可直接放置（不打开 App，放置成功有提示），点任意一行直达详情
- **数据备份**：导出 / 分享 / 导入备份文件，导入前逐条预览（跳过/覆盖/导入），备份附带主题设置，方便换机同步
- **主题与图标**：怀月藏星 Themed 单色图标（Android 13+ 壁纸取色，浅壁纸黑 on 白/深壁纸白 on 黑，形体区分主次），支持浅色/深色/跟随系统与动态取色（Material You）；UI 主题体系化——倒计时数字等宽（tabular-nums）+ display 字阶、全局圆角 4 档语义规格、次要文字对比度达 WCAG AA（4.6:1）
