...
- **桌面小组件**：2×2 / 4×2 / 4×4 三档尺寸，纸笺辰刻排版（左 3.5dp 色条 + 36dp 头像 + 类型标签 + 20sp 倒计时，≤7 天呼吸边框，缅怀灰蓝、空态「＋添加」按钮，暗色自适配）；可配置展示最近记录或指定某人；长按应用图标「添加小组件」或设置页「关于」入口可直接放置（不打开 App，放置成功有提示），点任意一行直达详情
