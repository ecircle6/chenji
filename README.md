...
- **桌面小组件**：2×2 / 4×2 / 4×4 三档尺寸，可配置展示最近记录或指定某人；长按应用图标「添加小组件」或设置页「关于」入口可直接放置（不打开 App，放置成功有提示）
...
- **CI**（`.github/workflows/ci.yml`）：每次 push/PR 自动运行全部单元测试（农历/备份/筛选/事件计算/闹钟调度/迁移/Compose UI/ViewModel/分层/Hero 候选共 26 个测试类 198 用例）+ 构建 Debug 包
