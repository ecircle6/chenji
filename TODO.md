...
- [x] **英文环境详情页崩溃 + i18n 语言口径分裂（UI 改版审计 P0）**（2026-09-06 修复）
  ...
  - 发版（2026-09-06）：versionCode 23→24 / versionName 2.1.20→2.1.21；`Changelog.kt` 头部加 v2.1.21 条目；模拟器实测升级首启弹窗渲染正常（`verify_20260906_131710.png`）、「知道了」关闭正常；assembleRelease 签名包构建通过（`辰记_v2.1.21.apk`）；GitHub Release 首次成功发布（资产 `chenji_v2.1.21.apk`，workflow 修障三连：heredoc 缩进 → 顶行破坏 YAML → 改 echo 组 + ASCII 名副本上传）
  - 发版后回归（2026-09-06，模拟器 v2.1.21 实测）：分享卡中/英两轮生成无崩溃（英文轮即 P0-4 实战场，旧代码此刻必崩）；新增→保存→删除全流程正常（新事件接管 Hero、删除后 zz 回归、分组排序与闹钟重排无异常）；动态取色开关生效（壁纸取色即时渲染）；日历翻月正常（农历换算正确）；小组件渲染正常（中文）。⚠ 小组件 resize 三档模拟未能完整复跑：此 AVD 广播队列状态异常（DEBUG_SIMULATE_RESIZE 与官方 APPWIDGET_UPDATE 均入队不分发，重启仅恢复一条），已实证接收器带注解改动正常执行一次（logcat optionsChanged + scheduled apply）、档位逻辑由 WidgetLayoutTest 15 用例锁定，真机拖拽终验仍待用户（原常设项）
...
- [x] **小组件拖拽缩放过程中新旧档位画面短暂重叠（叠影）**（2026-09-05 修复）
  ...
  - 修复：Receiver 端防抖——options 回调只更新 `WidgetSizeCache`（静态 Handler + 待应用表，按 widgetId 取消重挂），尺寸停止变化 300ms 后才 `refresh` 一次；不再调 `super`（逐次重组合正是叠影来源）。拖动中旧画面由 launcher 拉伸展示（各小组件标准行为），松手停稳后一次性切到新档位
  - 连带修正 `realWidgetSize` 缓存取值：防抖后只有停稳时一次重绘、无后续重绘可自愈，旧「仅当缓存更小时采用」在放大场景会拿滞后的旧小值卡住档位——改为新鲜缓存（5s 内）直接可信（缓存本就来自最后一次回调，比持久化 options 更新）
  - 追加修复（2026-09-05 晚，用户复测「缩小后仍显示多个通知」）：真正的根因是 **Glance 1.1.1 长活会话期间 `update()` 是空操作**（字节码实锤：`update` 先查 `SessionManager.isSessionRunning`，命中则走 `session.updateGlance()` = `sendEvent(UpdateGlanceState)` 只刷新 state datastore、不按新尺寸重组）。`provideContent` 挂着数据库 Flow 订阅、会话一直存活，于是进程启动后**只有第一次 resize 能渲染**，之后连续缩放全被吞掉（用户 10 秒内连拖 6 次只有第一次生效，停在宽档）；会话约 10 秒后随广播 PendingResult 过期自动关闭，自动化测试每轮间隔 30 秒以上恰好躲开——「测试总通过、用户实测失败」的原因。修复：尺寸改为**可观察状态**——`WidgetSizeCache` 按 widgetId 持有 `MutableStateFlow<IntSize?>`，Receiver 防抖停稳后写入，组合里 `collectAsState` 订阅，运行中的会话据此自动重组一次（与数据变化的重绘同一机制）；`updateAll` 保留用于无会话场景（冷进程首帧），活会话时无害空操作。顺带补 `goAsync()` 保活（冷进程防抖应用防回收）与 debug 模拟广播（`com.birthapp.DEBUG_SIMULATE_RESIZE`，免手势回归）
  - 验收：✅ 全量单测绿 + assembleDebug 通过；模拟器实测（会话存活期间连续缩放）——放大 `来源=状态流 266x135 → tier=WIDE`、缩小 `172x135 → tier=COMPACT`、窄而高 `172x281 → COMPACT`（宽度优先）、宽而高 `266x281 → tier=LARGE`（大档 5 行均分），每次停稳一次干净应用、无叠影；点行开 App、深色模式复验通过
