...
- [x] **小组件拖拽缩放过程中新旧档位画面短暂重叠（叠影）**（2026-09-05 修复）
  ...
  - 验收：✅ 全量单测绿 + assembleDebug 通过；模拟器实测——拖动中途停顿仅出现一次干净应用（正确渲染紧凑档、无叠影），重装刷新后按实际尺寸正确渲染单焦（options 缓存读值链路正确）；完整拖拽周期受环境限制未自动化跑通（launcher 调整框不理会合成输入、真鼠标自动化被其他桌面应用抢焦点打断），留真机拖拽复验
