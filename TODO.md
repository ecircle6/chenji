...
- [x] **小组件拖拽缩放过程中新旧档位画面短暂重叠（叠影）**（2026-09-05 修复）
  ...
  - 追加修复（2026-09-05 晚，用户复测「缩小后仍显示多个通知」）：防抖初版丢了 `super` 里的 `goAsync()` 保活——`onAppWidgetOptionsChanged` 若发生在**冷启动进程**（系统为这条广播专门拉起），`onReceive` 返回后进程立即可被回收，300ms 后的防抖应用随进程一起丢掉，桌面停留在旧档位。修复：自己 `goAsync()` 持有 `PendingResult`，refresh 完成后才 `finish()`；新回调取代旧任务时提前 finish 旧的；并补了全链路诊断日志（`logcat -s BirthWidget`：回调/防抖/应用/取值来源/档位）
  - 验收：✅ 全量单测绿 + assembleDebug 通过；模拟器实测（暖/冷进程、单格/跨两格、方向来回翻转共 6+ 轮拖拽）——每次停稳后均一次干净应用、档位正确（缩小→紧凑单焦、放大→多行）、无叠影；点行开 App、深色模式自动切夜间配色均复验通过。另：桌面端 launcher 的调整框不理会 adb 合成手势，缩放交互的回归只能真鼠标/真机做（自动化期间桌面被其他应用占用焦点未能跑完最后一轮，需用户手动复验一次缩放）
